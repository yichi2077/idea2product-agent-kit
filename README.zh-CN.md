...
- **PyYAML + pytest** — 仅 `verify`（维护者/CI 测试套件）需要。安装：
  macOS/Linux 用 `python3 -m pip install -r .pipeline/requirements-dev.txt`，
  Windows 用 `python -m pip install -r .pipeline/requirements-dev.txt`。
...
有两个 canonical 源，各自在别处镜像一份以实现自包含安装：

- `pipeline-template/` → 以 `assets/pipeline-template` 形式内嵌进 guided-flow 技能。
- `skills/` → 以 `agent-adapters/<宿主>/skills` 形式内嵌进 Hermes 和 OpenClaw 适配。

编辑**任一** canonical 源之后，重新镜像并对漂移做门禁检查：

```bash
python3 scripts/sync_bundles.py          # 把 canonical 传播到镜像
python3 scripts/sync_bundles.py --check  # 任一镜像漂移则非零退出（在 CI 中运行）
```

从已脚手架的工作区运行完整测试套件（macOS/Linux 用 `verify.sh`，Windows 用
`verify.ps1`）：
