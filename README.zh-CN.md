...
每次战略门禁都要求对推荐结论进行一次跨模型红队审查。

**建设前先查现成方案。** P2 会先搜索可直接使用的产品、服务、开源项目和替代工作流。
如果已经有方案足够解决这个 idea，Agent 会先告诉你，而不是继续推动你进入产品规划。
...
- **`skills/`** — 16 个可安装的入口技能：
  ...
  - `idea2product-p1-…` 到 `…-p9-outcome-review` — 每个阶段一个技能，另有 P2 内部的 `idea2product-p2-existing-solutions-scan`。
  - `idea2product-p0-status` / `idea2product-p0-resume` / `idea2product-p0-rollback` / `idea2product-p0-doctor` / `idea2product-p0-retire` — 报告 / 继续 / 回退 / 健康检查 / 放弃归档。
...
- **Python 3.10+** — 唯一需要的东西。安装器和所有流水线命令（`status`、`run`、
  `gate`、`stage`、`doctor`、`retire`）都只用 Python 标准库。不需要 PowerShell、不需要 Node、
...
这会把 16 个技能复制到 `~/.claude/skills`，Claude Code 会按名称自动发现它们。
若没有立即出现，新开一个会话即可。
...
idea2product-p0-rollback   回退一个已完成阶段（带确认的 reopen）
idea2product-p0-doctor     健康检查流水线状态和产出
idea2product-p0-retire     带确认和理由地放弃/归档项目
idea2product-p2-existing-solutions-scan
                            在 P2 继续前搜索可直接使用的现成方案
idea2product-p1-idea-expansion … idea2product-p9-outcome-review
```

如果状态或文件看起来不一致，让 Agent 使用 `idea2product-p0-doctor`。它会运行只读健康检查，
报告缺失产出、仍是 scaffold 的占位产出、已完成产出的 stale 警告，以及 gate/state invariant 问题。
如果你要放弃或归档项目，使用 `idea2product-p0-retire`；它必须先确认并记录理由，才会把未完成阶段标记为 retired。
