...
1. Start with `python .pipeline/scripts/pipeline.py status`.
2. Use `python .pipeline/scripts/pipeline.py resume` to determine the next action.
...
6. Run `.pipeline/scripts/verify.ps1` after changing pipeline files.

If `.pipeline/scripts/pipeline.py` is missing, the installed idea2product guided skill initializes the current workspace automatically before status, resume, or phase execution. Manual initialization is still available:
