...
2. **But the *de facto* design is real and strong.** Each phase has its own recipe, its own `outputs:`, its own thematic doc dir; the README sells *"focuses agent context only on the active phase"* (context isolation), and the WHY→WHAT→HOW split (P1-3 / P4-5 / P6-9) keeps phases single-concern. **Your instinct holds:** the candidate host phases (P4 discovery, P5 definition) are deliberately *pre-code thinking* phases — dropping a "build a throwaway + test on users" step inside them genuinely mixes building/testing into a definition phase and breaks the clean shift-left separation. **→ A dedicated phase is the correct call.**
3. **The engine treats every phase in `PHASES` as mandatory + strictly sequential** (`PHASES = ["P1".."P9"]` hardcoded in `pipeline.py`; `stage_complete` only completes the current `ready` phase, rejects skips, derives `next` by list index). **There is no "optional phase" concept.** So a new phase becomes required for every project → it should be **waivable via a recorded "skip with rationale"** (mirrors the existing-solutions-scan `User Decision` pattern), honoring your "no hard floor / lightweight" choice.
